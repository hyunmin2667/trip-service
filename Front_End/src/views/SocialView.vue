...
const customOverlayMap = new Map();
...
const setData = (data) => {
  ...
  //     date: new Date('2023-11-22 00:50:00')
  //   },
  //   {
  //     postId: 3,
  //     username: 'hong9932',
  ...
  //     date: new Date('2023-10-21 01:00:00')
  //   },
  //   {
  //     postId: 4,
  //     username: 'thisisid139aaaaaaaaa',
  //     title: '근린공원',
  //     latlng: new kakao.maps.LatLng(33.451393, 126.570738),
  //     image: 'https://picsum.photos/id/18/200/200',
  //     date: new Date('2022-10-22 12:30:00')
  //   }
  // ]
}
...
const createOverlayContent = () => {
  for (var i = 0; i < contentList.value.length; i++) {
    var postId = contentList.value[i].postId
    var username = contentList.value[i].username
    var title = contentList.value[i].title
    var latlng = contentList.value[i].latlng
    var image = contentList.value[i].image
    var date = formatDate(contentList.value[i].date)

    // 커스텀 오버레이를 생성합니다
    customOverlay.value[i] = new kakao.maps.CustomOverlay({
      position: contentList.value[i].latlng,
      content:
        `
        <div class="minicard" id="minicard${postId}" data-postid="${postId}"> 
    ...
    // 각 overlay 객체 접근을 위해 key를 postId로 가지는 Map에 저장
    customOverlayMap.set(postId, customOverlay.value[i])

    // z index 설정 방법
    customOverlay.value[i].setMap(map);
  }
...
// 게시물 ID 가져오기 TEST
const getpostId = (postId) => console.log(`postId를 가져옵니다 : ${postId}`);
